...
{
    class PuctuationCheck1 : ITextCheck
    {
         ...
         */
        static PuctuationCheck1()
        {
            PuctuationCheck1.checkRegex = new Regex("(?<=[;,]([\"'](\\s[\"'])*)?(\\\\f([^\\\\]|\\\\(?!f\\*))*?\\\\f\\*)*(\\s*\\\\\\w+)+(\\s*\\\\v\\s\\S+)?\\s+(\\\\x([^\\\\]|\\\\(?!x\\*))*?\\\\x\\*)?)[A-Z]\\w+",
                RegexOptions.Multiline | RegexOptions.IgnoreCase);
        }

        public PuctuationCheck1(TranslationValidationPlugin plugin, IHost host, string activeProjectName)
        {
            this.plugin = plugin;
            this.host = host;
        ...
        public void RunCheck()
        {
            try
            {
                ...
                Parallel.For<ExtractorState>(1, (MainConsts.MAX_BOOK_NUM + 1),
                    new ParallelOptions { MaxDegreeOfParallelism = MainConsts.MAX_CHECK_THREADS },
