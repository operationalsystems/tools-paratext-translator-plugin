...
namespace TvpTest
{
    ...
    public class ImportManagerTests : AbstractCheckTests
    {
        ...
        public void TestSetup()
        {
            ...
            var testVerseDictionary = new Dictionary<VerseLocation, string>();

            foreach (var lineText in File.ReadAllLines(@"Resources\test-usfm-1.txt"))
            {
                    ...
                    || workText.StartsWith(@"\v "))
                {
                    ...
                    else if (workText.StartsWith(@"\v "))
                    {
                        ...
                        Assert.IsFalse(testVerseDictionary.ContainsKey(verseLocation),
                            $"Duplicate verse location in test input: {verseLocation.VerseCoordinateText}");

                        testVerseDictionary[verseLocation] = verseBuilder.ToString();
                        verseBuilder.Clear();
            ...
            testVerseDictionary[new VerseLocation(TEST_BOOK_NUM, nextChapterNum, nextVerseNum)]
                = verseBuilder.ToString();
            VerseDictionary = testVerseDictionary;
